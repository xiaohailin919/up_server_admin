...
class ReportTcController extends BaseController
{
    /**
     * BI 返回参数表 data 的字段
     */
    ...
    const METRICS_QCC_CTR              = 'qcc_ctr';                // QCC CTR
        ...
        foreach ($this->requestParams as $param => $defaultValue) {
            ...
            switch ($param) {
                ...
                case self::METRICS_PLACEMENT_ID:
                    ...
                    if ($model->newQuery()->where('uuid', $input[$param])->count() === 0) {
                        return redirect()->back()
                            ->withInput($request->only(array_keys($this->requestParams)))
                            ->withErrors([$param => '未找到相应'. $this->selectableDimensions[$param] . ' ID 记录: ' . $input[$param]]);
                    }
                    $input[$param] = [$model->newQuery()->where('uuid', $input[$param])->first(['id'])['id']];
                    break;
                /* 将传入值改成 int 数组类型 */
...
//        return $input;
        ...
        $groupByMetricsMap = $this->selectableDimensions; // 复制所有可 group by 的维度
...
//        return $orderByStr;
        ...
        for ($i = 0, $iMax = count($this->inputFieldList); $i < $iMax; $i++) {
            ...
            $this->inputFieldList[$i]['placeholder'] = $this->selectableDimensions[$metric];
        ...
        for ($i = 0, $iMax = count($this->selectionList); $i < $iMax; $i++) {
            ...
            $this->selectionList[$i]['placeholder'] = $this->selectableDimensions[$metric];
        ...
        $limit = $request->get('export') === '1' ? 1000 : 25;
...
//        return $data;
        ...
        for ($i = 0, $iMax = count($data); $i < $iMax; $i++) {
            ...
            // 临时计算CTR
            $data[$i][self::METRICS_QCC_CTR] = 0;
            if($data[$i][self::METRICS_QCC_IMPRESSION] > 0){
                $data[$i][self::METRICS_QCC_CTR] = $data[$i][self::METRICS_QCC_CLICK] / $data[$i][self::METRICS_QCC_IMPRESSION];
            }
        }
        ...
        if ($request->get('export') === '1') {
            ...
            for ($i = 0, $iMax = count($selectedDimensions); $i < $iMax; $i++) {
                ...
                switch ($selectedDimensions[$i]) {
                    case self::METRICS_APP_ID:
                        $groupByMetricsMap[self::METRICS_APP_ID]         = '应用 ID';
                        $groupByMetricsMap[self::METRICS_APP_UUID]       = '应用 UUID';
                        $groupByMetricsMap[self::METRICS_APP_NAME]       = '应用名';
                        break;
                    case self::METRICS_PLACEMENT_ID:
                        $groupByMetricsMap[self::METRICS_PLACEMENT_ID]   = '广告位 ID';
                        $groupByMetricsMap[self::METRICS_PLACEMENT_UUID] = '广告位 UUID';
                        $groupByMetricsMap[self::METRICS_PLACEMENT_NAME] = '广告位名称';
                        break;
                    case self::METRICS_UNIT_ID:
                        $groupByMetricsMap[self::METRICS_UNIT_ID]        = '广告源 ID';
                        $groupByMetricsMap[self::METRICS_UNIT_NAME]      = '广告源名称';
                        break;
                    case self::METRICS_PUBLISH_ID:
                        $groupByMetricsMap[self::METRICS_PUBLISH_ID]     = '开发者 ID';
                        $groupByMetricsMap[self::METRICS_PUB_NAME]       = '开发者名称';
                        break;
                    default:
                        $groupByMetricsMap[$selectedDimensions[$i]] = $this->selectableDimensions[$selectedDimensions[$i]];
                }
            }
            /* 处理表格数据 */
            for ($i = 0, $iMax = count($data); $i < $iMax; $i++) {
                ...
                /* 清除表头不需要的数据 */
                foreach ($this->selectableDimensions as $metric => $name) {
...
//            return $data;
            $headers = array_merge($groupByMetricsMap, $this->fixMetricsMap);
...
//        return $report;
        ...
        return view('report-tc.index')
            ...
            ->with('selectionList', $this->selectionList)
