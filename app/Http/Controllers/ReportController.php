...
//        return (int)User::isChannel(Auth::id());

//        if(!User::isChannel(Auth::id())){
            return redirect('/report-full');
//        }
        ...
        $publisherIdResult =  $this->getPublisherIdList($where);
        Log::info("dd:" .json_encode($publisherIdResult));
...
//        $placementList = $this->getIdByField(new Placement(), "id", "uuid", $where["placement_uuid"]);
        $placementList  = [];
        ...
        $isAdminInChannelList = ChannelAdapter::isInChannelList(Auth::id());
        $requestBi = true;
        if($isAdminInChannelList){
            if(empty($publisherIdResult['all']['include1']) && empty($publisherIdResult['all']['include2']) && empty($publisherIdResult['all']['include3']) && empty($publisherIdResult['publisher_id_intersect'])) {
                Log::info("inChannelMode, but_publisher_id_all_empty, will fill it");
                $requestBi = false;
                $publisherModel = new Publisher();
                $publisherBuilder = $publisherModel->queryBuilder();
                $this->adaptModelQueryBuilder($publisherModel, $publisherBuilder);
                $publisherIdList = array_column($publisherBuilder->get()->toArray(), "id");
                Log::info("fill_result:" . json_encode($publisherIdList));
                if(!empty($publisherIdList)){
                    $publisherIdResult['all']['include1'] = $publisherIdList;
                    $publisherIdResult['all']['include2'] = [];
                    $publisherIdResult['all']['include3'] = [];
                    $publisherIdResult['publisher_id_intersect'] = $publisherIdList;
                    $requestBi = true;
                }
            }
        } else if($channel != "all"){
        ...
        if($requestBi){
            $urlParam = [
                ...
                //多选
                "contains" => (object)[
                    "publisher_id"           => $publisherIdResult['publisher_id_intersect'], // $publisherIdResult['all'],
//                    "publisher_id_intersect" => $publisherIdResult['publisher_id_intersect'],
                    "app_id"                 => !empty($appIdList["include2"]) ? $appIdList["include2"] : $appIdList["include1"],// $appIdList,
                    "area"                   => $geo,
...
//                    "sdk_version" => $where["tk_sdk_version"],
//                    "app_version" => $where["tk_app_version"],
                ],
            ];
        ...
        if($isAdminInChannelList){
            ...
            $tableFields = array_merge($tableFilter, $tableFields);
        }
        ...
        foreach ($list["list"] as $k => &$v) {
            ...
            if(!$isAdminInChannelList && isset($v['publisher_id']) && isset($v['publisher_name'])){
                $v['publisher_name'] .= '<br /><a href="/publisher/login?id=' . ($v['publisher_id']) .
                    '" target="_blank"><small><i class="mdi mdi-login"></i> Login</small></a>';
            }
        ...
        if ($export == 1) {
            ...
            Export::excel($header, $list['list'], true, $this->timezoneList[$timezone] . "_export");
            exit;
        }
        ...
        return view(ChannelAdapter::getViewFile(ChannelAdapter::PAGE_REPORT_INDEX, Auth::id()))
