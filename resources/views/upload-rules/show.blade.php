...
@section('content')
            ...
            <div class="page-title-box">
                <div class="btn-group pull-right">
                    <ol class="breadcrumb hide-phone p-0 m-0">
                        ...
                        <li class="breadcrumb-item active">Edit Upload Rules</li>
                    </ol>
                </div>
                <h4 class="page-title">Edit Upload Rules</h4>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card-box">
                {{ Form::model($data, array('route' => array('upload-rules.update', $data['id']), 'method' => 'PUT')) }}
                @if ($data['rule_type'] === \App\Models\MySql\StrategyAppLogger::RULE_TYPE_APP)
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            <label>App ID</label>
                            <input type="text" class="form-control" readonly value="{{ $data['app_uuid'] }}" name="app_uuid" placeholder="请填写App Uuid">
                        </div>
                    </div>
                @else ($data['publisher_group_id'] === \App\Models\MySql\StrategyAppLogger::RULE_TYPE_PUBLISHER_GROUP)
                    <div class="form-row">
                        <div class="form-group col-md-8">
                            <label>Publisher Group Id</label>
                            <input type="text" class="form-control" readonly value="{{ $data['publisher_group_id'] }}" name="app_uuid" placeholder="Publisher Group Id">
                        </div>
                    </div>
                @endif
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="form-group col-md-8" id="da_not_keys">
                        ...
                        <button class="btn btn-sm btn-primary" style="float: right" type="button" onclick="onAddClick(this)">添加</button>
                        <div class="form-row title-row" @if(count($data['da_no_report_keys_by_rate']) === 0) hidden @endif>
                            <div class="col-md-3">
                                <label><strong>Key</strong></label>
                            </div>
                            <div class="col-md-6">
                        ...
                        @if (count($data['da_no_report_keys_by_rate']) === 0)
                            <div class="form-row content-row" hidden>
                                <div class="col-md-3">
                        ...
                        @else
                            @php($index = 0)
                            @foreach($data['da_no_report_keys_by_rate'] as $daNotKeys => $daNotKeysFormatRate)
                                <div class="form-row content-row">
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-6">
                                        <select name="da_not_keys_ft[{{ $index }}][]" class="form-control select2 select2-multiple select2-hidden-accessible" multiple="" data-placeholder="-- 广告类型 --" required>
                                            @foreach ($formatMap as $key => $val)
                                                <option value="{{ $key }}" @if(in_array($key, $daNotKeysFormatRate['formats'], false)) selected @endif>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="input-group">
                                            ...
                                            <input class="form-control" name="da_not_keys_rates[]" value="{{ $daNotKeysFormatRate['rate'] }}" type="number" placeholder="Rate">
                                        </div>
                                    </div>
                                    <div class="col-md-1">
                                        <i class="mdi mdi-close-circle" onclick="onDelClick(this)"></i>
                ...
                <div class="form-row">
                    <div class="form-group col-md-8" id="tk_n_t">
                        ...
                        <button class="btn btn-sm btn-primary" style="float: right" type="button" onclick="onAddClick(this)">添加</button>
                        <div class="form-row title-row" @if(count($data['tk_no_report_keys_by_rate']) === 0) hidden @endif>
                            <div class="col-md-3">
                        ...
                        @if (count($data['tk_no_report_keys_by_rate']) === 0)
                            <div class="form-row content-row" hidden>
                                <div class="col-md-3">
                        ...
                        @else
                            @php($index = 0)
                            @foreach($data['tk_no_report_keys_by_rate'] as $tkNoType => $tkNoTypeFormatsRates)
                                <div class="form-row content-row">
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-6">
                                        <select name="tk_n_t_ft[{{ $index }}][]" class="form-control select2 select2-multiple select2-hidden-accessible" multiple="" data-placeholder="-- 广告类型 --" required>
                                            @foreach ($formatMap as $key => $val)
                                                <option value="{{ $key }}" @if(in_array($key, $tkNoTypeFormatsRates['formats'], false)) selected @endif>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="input-group">
                                            ...
                                            <input class="form-control" name="tk_not_keys_rates[]" type="number" value="{{ $tkNoTypeFormatsRates['rate'] }}" placeholder="Rate">
                                        </div>
                                    </div>
                                    <div class="col-md-1">
                                        <i class="mdi mdi-close-circle" onclick="onDelClick(this)"></i>
                ...
                <div class="form-row">
                    <div class="form-group col-md-8">
                        ...
                        <input type="number" class="form-control" name="tcp_port" value="{{ $data['tcp_port'] }}" min="0" max="65535">
                    </div>
                </div>
                <div class="form-row">
                    <div class="form-group col-md-8">
                        ...
                        <div>
                            <label class="custom-control custom-radio">
                                <input class="custom-control-input" name="tcp_tk_da_type" type="radio" value="1" @if($data['tcp_tk_da_type'] === 1) checked @endif>
                                <span class="custom-control-indicator"></span>
                                <span class="custom-control-description">只上报一种协议</span>
                            </label>
                            <label class="custom-control custom-radio">
                                <input class="custom-control-input" name="tcp_tk_da_type" type="radio" value="2" @if($data['tcp_tk_da_type'] === 2) checked @endif>
                                <span class="custom-control-indicator"></span>
                ...
                <div class="form-row">
                    <div class="form-group col-md-3">
                        <label>Status</label>
                        <select name="status" class="form-control" @if ($data['rule_type'] === \App\Models\MySql\StrategyAppLogger::RULE_TYPE_APP && $data['app_uuid'] === 0) readonly @endif>
